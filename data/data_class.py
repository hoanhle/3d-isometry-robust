...
def modelnet40_download():
    ...
    data_dir_path = os.path.join(base_dir_path, "modelnet40_data")
    if not os.path.exists(data_dir_path):
        os.mkdir(data_dir_path)
    if not os.path.exists(os.path.join(data_dir_path, "modelnet40_ply_hdf5_2048")):
        www = "https://shapenet.cs.stanford.edu/media/modelnet40_ply_hdf5_2048.zip"
        zip_file = os.path.basename(www)
        os.system("wget %s; unzip %s" % (www, zip_file))
        os.system("mv %s %s" % (zip_file[:-4], data_dir_path))
...
def load_data(partition):
    ...
    for h5_name in glob.glob(
        os.path.join(data_dir_path, "ply_data_%s*.h5" % partition)
    ):
        f = h5py.File(h5_name)
...
class ShapeNetPart(Dataset):
    def __init__(
        ...
        splitfile = os.path.join(
            self.root,
            "train_test_split",
            "shuffled_{}_file_list.json".format(partition),
        )
            ...
            if category in self.cat.values():
                self.meta[self.id2cat[category]].append(
                    (
                        ...
                        os.path.join(
                            self.root, category, "points_label", uuid + ".seg"
                        ),
                    )
                )
